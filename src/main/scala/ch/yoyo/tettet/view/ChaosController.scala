...
@sfxml
class ChaosController(tetris: AnchorPane, tetrisBoard: GridPane,
  ...
  val cTetrad = new ChaosTetrad()
  val salTetrad  = new SalvatorTetrad()

  //hold all instances
  val tetradLst = List(iTetrad, jTetrad, lTetrad, tTetrad, oTetrad, zTetrad, sTetrad, cTetrad, salTetrad) //refer
  // hold all the tetrad
  val tetrad = List(iTetrad.block, jTetrad.block, lTetrad.block, tTetrad.block, oTetrad.block, zTetrad.block, sTetrad.block, cTetrad.block, salTetrad.block )
  ...
  def randomPiece(): List[List[Array[Int]]] = {
    return tetrad(Random.nextInt(tetrad.size))
  }
