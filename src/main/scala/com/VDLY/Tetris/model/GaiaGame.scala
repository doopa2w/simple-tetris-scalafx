...
class GaiaGame extends ScalaFXSound {
  ...
  //list of tetrads
  val tetrads = List(Tetramino.T, Tetramino.J, Tetramino.L, Tetramino.O, Tetramino.Z, Tetramino.S, Tetramino.T, Tetramino.B)
  ...
  def moveSet(x: Int, y: Int): Unit = {
    for (a <- currentPiece.indices) {
      ...
      if ((currentPiece(a)(1) + tetraminoes.currentY + y) > gameBoard.rows - 1) {
        gameBoard.paintBoard(Tetramino.B, currentPiece, tetraminoes.currentX + x, tetraminoes.currentY)
        currentPiece = List()
        addScore(1)
        collisionSoundEffect()
        return
